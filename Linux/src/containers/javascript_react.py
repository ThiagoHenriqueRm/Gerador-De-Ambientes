...
def Javascript_React():
    ...
    def CriarProjeto():
        ...
        # Tenta criar o projeto
            ...
            # Criação do Projeto
            DirProjeto = MK_react(nome, dir, clear)
            statusLabel.configure(
                    text="Projeto Criado! \n" + f"{DirProjeto}",
                    text_color="#138D19"
                )
    ...
    checkbox_pasta = CTK.CTkCheckBox(
        ...
        border_width    = 2,
        corner_radius   = 5
    );checkbox_pasta.pack(pady=0, anchor="w", padx=10)


    # Checkbox: Limpar Projeto
    checkbox_clear = CTK.CTkCheckBox(
        checkbox_width  = 18,
        checkbox_height = 18,
        master          = ChecFrame,
        text            = "GERAR UM PROJETP LIMPO?",
        font            = FONTE,
        border_width    = 2,
        corner_radius   = 5
    );checkbox_clear.pack(pady=(1, 10), anchor="w", padx=10)
